...
# MODULE DEVELOP METHODOLOGY
               ...
               2 - Writing the metasploit template: librarys, class name, mixins, description (def initialize)
...
![msf-auxiliarys](http://i.cubeupload.com/ACpzcT.png)<br />
      ...
      Methods are defined using the keyword def followed by the method name. Method parameters are specified between parentheses following the method name. The method body is enclosed by this definition on the top and the word end on the bottom. By convention
method names that consist of multiple words have each word separated by an underscore.
