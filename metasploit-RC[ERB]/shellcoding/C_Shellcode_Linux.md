...
The shellcode data its now stored inside a bash local variable named **$pa**. This variable its going to be called later to embebbed the shellcode into our C Program while we are creating it using the follow commands described in the next grey box(s).<br />
