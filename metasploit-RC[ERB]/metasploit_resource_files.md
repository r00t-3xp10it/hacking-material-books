...
<blockquote>The next resource script uses db_nmap metasploit core command to populate msfdb database with hosts, then the ruby function will check what hosts has been capture and run 3 post-exploitation modules againts all hosts stored inside the msfdb database.</blockquote>
